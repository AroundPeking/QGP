...
!------------------------We only need to change the parameters here------------------------------------------
    ...
    !System of collision
    ISys = 2 !  =1 for PbPb at 2.76, =2 for PbPb at 5.02
...
!	include 'Recom.h'
	...
	real*8 meson_TS
	real*8 pt, C_M, gamc,gaml,gams,resu,tmp,x,num
...
!-----------------------------------------------------------		for D0
    elseif(IDmeson.eq.2)then !for D0
        ...
        do i=1,num
            ...
            +S_j(pt*tmp,IDP2)*Thermal(pt*(1-tmp),IDP)/gamc/(1-tmp) )*(1-tmp)**4)*(1.-0.)/num/2
        enddo
...
!-----------------------------------------------------------		for Ds
	...
	do i=1,num
            ...
            +S_j(pt*tmp,IDP2)*Thermal(pt*(1-tmp),IDP)/gamc/(1-tmp) )*tmp**2 *(1-tmp)**9)*(1.-0.)/num/2
        enddo
...
!	include 'Recom.h'
	external int_SS,S_j
	real*8 meson_SS
	real*8 pt,q,m_h,p0,tmp, q_next,fdel
...
!------------------------------------------------------------	for J/psi
	if(IDmeson .eq.1)then !for J/psi
        ...
        !    q1=fa+i*(fb-fa)/ni
        !    q2=q1+(fb-fa)/ni
        !    fsum1=fsum1+(Int_HardInt(q1, pt/2., IDP1)+Int_HardInt(q2, pt/2., IDP1))/2.*(fb-fa)/ni
        !enddo
        !do j=1,ni
        !    k1=fa+j*(fb-fa)/ni
        !    k2=k1+(fb-fa)/ni
        !    fsum2=fsum2+(Int_HardInt(k1, pt/2., IDP2)+Int_HardInt(k2, pt/2., IDP2))/2.*(fb-fa)/ni
        !enddo
        fsum=fsum1*fsum2
	...
	elseif(IDmeson.eq.2)then !for D0
        ...
        do j=1,ni
            ...
            do i=1,ni
                q1=fa+i*(fb-fa)/ni
                q2=q1+(fb-fa)/ni
                fsum1=fsum1+(int_SS2j(pt, q1, k1, IDmeson)+int_SS2j(pt, q2, k1, IDmeson))/2.*(fb-fa)/ni
                fsum2=fsum2+(int_SS2j(pt, q1, k2, IDmeson)+int_SS2j(pt, q2, k2, IDmeson))/2.*(fb-fa)/ni
            enddo
...
!----------------------------------------
    ...
    integer IDP2,i
    external int_Sj
    double precision pt, S_j, fa, fb, fdel, int_Sj, fsum
    real*8 q1,q2
    ...
    !    q1=fa+i*(fb-fa)/1000
    !    q2=q1+(fb-fa)/1000
    !    S_j=S_j+( int_Sj(pt,q1,IDP2)+int_Sj(pt,q2,IDP2) )*(fb-fa)/1000/2
    !enddo
...
!    end!function fik(k,IDP) end
    ...
    function fik(k,IDP)!2.76 and 5.02
    use Iflag
    implicit none
    real*8 Ai, ki, ni
    double precision k, fik
...
!    end!function fik(k,IDP) end
    ...
    implicit none 
    external S_ij, F1iq, int_DD0c, int_DD0g, int_DDsc, int_DDsg, int_Djpsic, int_Djpsig, SPD
    real*8   S_ij, int_DD0c,int_DD0g,x1,tmp, int_DDsc, int_DDsg, int_Djpsic, int_Djpsig, SPD
...
!-----------------------------------------------Jpsi--------------------------------------
    if(IDmeson.eq.1)then
       ...
       ! enddo
        ...
        int_SS =( (F1iq(q,8)*Dc)*4+(F1iq(q,5)*Dg)  )/q**2
...
!------------------------------------------------------
	...
	external F1iq, S_ij, QuenchSPD
	real*8 F1iq, S_ij, QuenchSPD
...
!------------------------------------------------------	
	...
	!do IDH = 1, 9
	IDH=5
	Int_HardInt = Int_HardInt + F1iq(q, IDH)*S_ij(pt/q, IDH, IDP)*QuenchSPD(pt)/q
	IDH=8
	Int_HardInt = Int_HardInt + F1iq(q, IDH)*S_ij(pt/q, IDH, IDP)*QuenchSPD(pt)/q
	!enddo
...
!	subroutine Integral_2d(fnx, pt, IDP, sumf)
!	implicit none
!!-------------------------------------------------	
!	external fnx
!	real*8 fnx
