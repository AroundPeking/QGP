...
mat=readtable('dNptdpt_PbPb276_Jpsi_c025.txt');
mat=table2array(mat);
%%前向快度区，不符合mid-rapidity
...
Cc=1.4;
Tc=0.7;
TT1=(Cc^2*pt_Exper./sqrt(pt_Exper.^2+3.097^2).*exp(-pt_Exper/Tc)/4)'*0.04;
TS=mat(:,3);
SS=mat(:,4);
SS2j=mat(:,5);
...
legend(['ALICE J/\psi@2.76 TeV' newline '(2.5<y<4.0)'],'total','TT','TS','SS^{1j}','SS^{2j}')%,'SS^{2j}(v15)'
