...
!	include 'Recom.h'
    ...
    do i=1,500
        ...
        do j=1,500
            ...
            k2=k1+(fb-fa)/500.
            fsum1=fsum1+( int_SS2j(pt,q1,k1,IDmeson)+   &
                          int_SS2j(pt,q1,k2,IDmeson) )*(fb-fa)/500./2.
            fsum2=fsum2+( int_SS2j(pt,q2,k1,IDmeson)+   &
                          int_SS2j(pt,q2,k2,IDmeson) )*(fb-fa)/500./2.
        enddo
        fsum=fsum+(fsum1+fsum2)/2.*(fb-fa)/200.
        fsum1=0.0d0
        fsum2=0.0d0
    ...
    do i=1,500
        ...
        do j=1,500
            ...
            fsum2=fsum2+( int_SS2j(pt,q2,k1,IDmeson)+   &
                          int_SS2j(pt,q2,k2,IDmeson) )*(fb-fa)/500./2.
        enddo
        fsum=fsum+(fsum1+fsum2)/2.*(fb-fa)/500.
        fsum1=0.0d0
        fsum2=0.0d0
    enddo
...
!---------------------------------------------------
    ...
    elseif(IDP2 .eq. 3)then!S_ubar
        do i=1,1000
            q=i*(fb-fa)/1000
            S_j=S_j+( int_Sj(pt,fa+q,IDP2)+int_Sj(pt,fa+q+(fb-fa)/500,IDP2) )*(fb-fa)/1000/2
        enddo
...
!    end!function fik(k,IDP) end
   ...
   !write(*,*)x,x1,int_DD0c
    ...
    elseif(IDmeson.eq.3)then!Ds
        ...
        int_SS2j =(F1iq(q, 5)*F1iq(k, 5)*fs5s5+  F1iq(q, 5)*F1iq(k, 8)*fs5s8+   &
                   F1iq(q, 8)*F1iq(k, 5)*fs8s5+  F1iq(q, 8)*F1iq(k, 8)*fs5s8)/q/k
