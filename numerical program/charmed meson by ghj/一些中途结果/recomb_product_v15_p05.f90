!Huanjing Gong change:
!	To calculate Jpsi by changing phi program: 
!		p0:m_Jpsi=3.096
!		fik:index 6、7 are note changed, however, fik has been changed.
...
! scan the parameter space
	...
	open(unit=101, file='./Jpsi276_fit.dat', status='old')
	open(unit=102, file='Jpsi276_chisq.dat',status='unknown')
...
! We only need to change the parameters here
	...
	Isu = 6 ! 1 for pion, 2 for Kaon, 3 for proton, 4 for lambda^0, 5 for Xi, 6 for Phi, 7 for Omega
	IDparticle = Isu
...
!------------------------------------------------------
	...
	external Fiqc, QuenchFiqc, SPD, QuenchSPD, S_ij
	real*8 Fiqc, QuenchFiqc, SPD, QuenchSPD, S_ij
...
!------------------------------------------------------	
    ...
    !print *, 'q:',x
	!print *, 'Fiq gluon:',Fiqc(5, x, centr)!*QuenchFiqc(x)*S_ij(p1/x, 5,IDP1)*QuenchSPD(p1)/x
    !print *, 'Fiq charm:',Fiqc(6, x, centr)!*QuenchFiqc(x)*S_ij(p1/x, 8,IDP1)*QuenchSPD(p1)/x
    !Intf_S1J + Fiqc(IDH, x, centr)*QuenchFiqc(x)*SPD(IDH, IDP1, p1/x)*QuenchSPD(p1)/x
