...
! scan the parameter space
	...
	open(unit=101, file='AuAu39_fit.Omega.dat', status='old')
	open(unit=102, file='AuAu39GeV_chisq.dat',status='unknown')
...
! We only need to change the parameters here
	...
	Npart = 341.7D0    ! N participant from experimental data

	Isu = 7 ! 1 for pion, 2 for Kaon, 3 for proton, 4 for lambda^0, 5 for Xi, 6 for Phi, 7 for Omega
	IDparticle = Isu

    IYangZhu = 2 ! =1 use Yang's equations,  =2 use Zhu's equations
! =1 for AuAu,=7 for AuAu at 39 Gev, =2 for PbPb at 2.76, =3 for PbPb at 5.02, =4 for XeXe at 5.44, =5 for dAu, =6 for pPb
    ISys = 7  
...
! for T as slope and c as coefficient
...
!         Ts = 0.545D0
!         cS = 11.0D0    ! c=0.025
! For the parameters to be tested
          T = 0.249D0
          Ts= 0.327D0
...
!#### cT and cS are detimined by IPARL what?

	DO IPAR1 = 0, 0

	cT = 37.2739d0 + REAL(IPAR1)*0.2d0

	DO IPAR2 = 0, 0

	cS = 11.537d0 + REAL(IPAR2)*0.1d0
...
!	print *, pt, dNptdpt(2),  dNptdpt(3)
	...
	WRITE(102, *)cS,PAR(1), PAR(2), CHISQ
...
! For Omega by Lilin
	...
	print *,f_OmegaTTS(pt, IDP1, IDP2, IDP3, centr)
	endif
...
! A cut-off for shower parton distribution
    ...
    elseif(IYangZhu.eq.2.and.ISys.eq.7)then
	QuenchSPD = 1.0D0 - dexp(-(p1/0.3D0)**2)  !#### for AuAu at39.6Gev
...
!---------------------------------------------------------------------------------------
    elseif(ISys.eq.1)then                    !I don't know what that is
...
!========================	
! for AuAu at 39.6 TeV system
! Logarithmic interpolation of the parameters between #####?????.
!========================
    
!#####
if(ISys.eq.7)then
	
!	print *, 'system: AuAu at 39.6 Gev'
!-----------------------------	
	select case(IDH)
! u quark		
	case(1)
