...
!------------------------We only need to change the parameters here------------------------------------------
    ...
    !System of collision
    ISys = 1 !  =1 for PbPb at 2.76, =2 for PbPb at 5.02
...
!-----------------------------------------------------------		for D0
    ...
    meson_TS = (1-dexp(-pt/2.0))*C_M/g *5*fsum
	
!-----------------------------------------------------------		for Ds
    elseif(IDmeson.eq.3)then ! for Ds
	
    call Integral_1d(int_mesonTS,fa,fb,fdel,pt,IDmeson,fsum)
    meson_TS = (1-dexp(-pt/2.0))*C_M/g *660*fsum
...
!------------------------------------------------------------	for D0
	elseif(IDmeson.eq.2)then !for D0
        ...
        do j=1,ni
            ...
            call Integral_intSS2j(int_SS2j,fa,fb,fdel,pt,k2,IDmeson,fsum1)!integral over q
...
!----------------------------------------
    ...
    if(IDmeson.eq.2)then
        IDP1=8
        IDP2=3
        int_mesonTS=( Thermal(pt*x,IDP2)*S_j(pt*(1-x),IDP1)/gaml/x              &
                    +S_j(pt*x,IDP2)*Thermal(pt*(1-x),IDP1)/gamc/(1-x) )*(1-x)**4
    elseif(IDmeson.eq.3)then
        IDP1=8
        IDP2=7
        int_mesonTS=( Thermal(pt*x,IDP2)*S_j(pt*(1-x),IDP1)/gams/x              &
            +S_j(pt*x,IDP2)*Thermal(pt*(1-x),IDP1)/gamc/(1-x) )*x**2 *(1-x)**9
    endif
