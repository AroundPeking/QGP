...
%% evaluate dis & Bh & chisq
% % clc
% % HH=["Xi","Lambda","proton","phi","Omega"];
% % EE=["7_7","11_5","19_6","27_","39_"];
% % CC=["05","1020","2030","3040","4060"];
% % 
% % %     CHISQ_min=2.733290e+01;
% % %     Tq0_min=0.39;
% % %     Ts0_min=0.51;
% % %     CHISQ_min=17.373599824071466;
% % %     Tq0_min=0.40;
% % %     Ts0_min=0.48;
% % %       CHISQ_min=1.724457e+01;
% % %       Tq0_min=0.400000;	
% % %       Ts0_min=0.476000;
% %       CHISQ_min=1.724436e+01;
% %       Tq0_min=0.399900;	
% %       Ts0_min=0.476200;	
% %  for iTq0=0:2000
% %  for iTs0=0:2000
% %   Tq0=0.3999+(iTq0-1000)*0.0001;
% %   Ts0=0.4762+(iTs0-1000)*0.0001;
% %     CHISQ=0;
% % for iHH=1:5
% %     H=HH(iHH);
% %     for iEE=1:5
...
% %                 eval(join(['[dis_',H,"_",E,"_",C,',Bh_',H,"_",E,"_",C,',chisq_',H,"_",E,"_",C,']=dis_Bh_func('...
% %                     ,'pt_',H,"_",E,"_",C,','...
% %                     ,'dNdpt_',H,"_",E,"_",C,','...
% %                     ,iHH,',',iEE,',',iCC,');'],''))
% %                 CHISQ=CHISQ+eval(join(['chisq_',H,"_",E,"_",C],''));
% %             else
% %                 %fprintf("%d\t%d\t%d\t%s\n",iHH,iEE,iCC,C)
% %             end
% %         end
...
% %         Tq0_min=Tq0;
% %         Ts0_min=Ts0;
% %     %fprintf("Tq0=%f\tTs0=%f\tCHISQ=%e\n",Tq0,Ts0,CHISQ)
% %     end
% %  end
% %  end
% %  fprintf("\nTq0=%f\tTs0=%f\tCHISQ=%e\n",Tq0_min,Ts0_min,CHISQ_min)
%% final evaluate for the follow plot
Tq0=0.400;
Ts0=0.476;
HH=["Xi","Lambda","proton","phi","Omega"];
EE=["7_7","11_5","19_6","27_","39_"];
...
for IH=1:5
    ...
    for IE=1:5
        ...
        for IC=1:5
            ...
            if exist(join(['pt_',H,'_',E,'_',C],''),'var')==0
                continue
            end
            pt=eval(join(['pt_',H,'_',E,'_',C],''));
            Bh=eval(join(['Bh_',H,'_',E,'_',C],''));
            sNN=11.5/1000;  
        ...
        [m0,ns]=h_prop(2);
        Th=3/((3-ns)/Tq+ns/Ts);
        mdl = @(beta,pt) beta(1)*exp(-pt/Th);
        beta0=[1];
        [beta,r,J,Sigma,mse,errorparam,robustw] =nlinmultifit(pt,Bh,mdl,beta0);
        fprintf('%d',beta)
        Bh=beta*exp(-pt/Th);
        eval(join(['Bh_',H,'_',E,'_',C,'=Bh'],''));
        end
    end
end
%% TEST plot
...
%    imwrite(fpic.cdata,join([pwd,'\image\AuAu ',Energy(iE),' GeV_Tq0=',num2str(Tq0),' & Ts0=',num2str(Ts0),'.jpg'],''));
end
%%
% global Tq0
% global Ts0
