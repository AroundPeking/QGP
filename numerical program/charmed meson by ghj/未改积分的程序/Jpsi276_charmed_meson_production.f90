...
!------------------------We only need to change the parameters here(end)------------------------------------------
    if(ISys.eq.1)then
    CollSys = 'PbPb276'
...
!    end!function fik(k,IDP) end
    ...
    if( (ISys.eq.1).and.(IDParticle.eq.1) )then!Jpsi276
        if(IDP.eq.5)then
            bL=2.39!2.76	Jpsi
...
!------------------------------------------------------
	...
	external F1iq, SPD, QuenchSPD
	real*8 F1iq, SPD, QuenchSPD
...
!------------------------------------------------------	
	...
	!do IDH = 1, 9
	IDH=5
	Int_HardInt = Int_HardInt + F1iq(q, IDH)*SPD(IDH, IDP, pt/q)*QuenchSPD(pt)/q
	IDH=8
	Int_HardInt = Int_HardInt + F1iq(q, IDH)*SPD(IDH, IDP, pt/q)*QuenchSPD(pt)/q
	!enddo
