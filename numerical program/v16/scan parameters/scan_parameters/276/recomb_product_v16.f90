...
elseif(ISys.eq.3)then
 ! parameters for Pb-Pb collisions at 5.02 TeV ! lilin May-16-2020
		 ...
		 !Tc & Cc
		 Tc = 0.83d0!0.8d0!0.9d0
		 cc = 2.2d0!2.4d0!2.1d0    ! c=0.025
         !Tc = 0.5D0+IPAR1*0.1d0
...
! D^0
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_D0_c025.txt", status = "unknown")
...
!------------------------------------------------
	...
	meson_TS = 660.0D0*gbst/p0/pt**13*fsumf
...
!---------------------------------------------	
	...
	meson_SS2j = 5.0d0*gbst*gamma2j/p0/pt**6*fsumf
    
    elseif(Isu.eq.10)then
	
	call Integral_1d(Intf_DsSS2j, fa, fb, fdel, fp1, fp2, fp3, fpt, fIDP1, fIDP2, fIDP3, fcentr, fsumf)
	meson_SS2j = 660.0d0*gbst*gamma2j/p0/pt**13*fsumf
...
!--------------------------------------------
	...
	Intf_DsSS2j = x**2*(pt-x)**9*HardInt(IDP1, x, centr)*HardInt(IDP2, pt-x, centr)
