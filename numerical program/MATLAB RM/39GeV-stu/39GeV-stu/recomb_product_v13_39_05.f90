...
! We only need to change the parameters here
	...
	Npart = 351.4D0   ! N participant from experimental data 0-5%
!    Npart = 325.2D0   !0-10%

	Isu =2 ! 1 for pion, 2 for Kaon, 3 for proton, 4 for lambda^0, 5 for Xi, 6 for Phi, 7 for Omega
	IDparticle = Isu

    IYangZhu = 2 ! =1 use Yang's equations,  =2 use Zhu's equations
    ISys = 1 ! =1 for AuAu, =2 for PbPb at 2.76, =3 for PbPb at 5.02, =4 for XeXe at 5.44, =5 for dAu, =6 for pPb
...
! parameters for Au-Au collisions at 39 GeV
	T = 0.260D0
	cT = 35.0D0    ! c=0.05
...
!    cT = 23.2D0    ! c=0.025
!    cS = 11.7D0    ! c=0.025
!    cS = 11.0D0    ! c=0.025, HZ2018-Eq.(24)
...
! pion
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_pion2_c025.txt", status = "unknown")
...
! kaon
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_kaon8_c025.txt", status = "unknown")
...
! proton
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_p3_c025.txt", status = "unknown")
...
! lambda^0
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_lambda7_c025.txt", status = "unknown")
...
! Xi
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_Xi2_c025.txt", status = "unknown")
...
! phi
	...
	open(unit = 11, file = "dNptdpt_"//CollSys//"_phi2_c05.txt", status = "unknown")
...
! Omega
    ...
    open(unit = 11, file = "dNptdpt_"//CollSys//"_Omega2_c05.txt", status = "unknown")
...
!    pt = 0.4D0+0.1D0*(i-1) ! n=17 for proton at low pt
    pt = 0.25D0+0.1D0*(i-1) ! n=18 for pion, kaon at low pt
!    pt = 0.5D0+0.3D0*(i-1) ! n=18 for Lambda for low pt
!    pt = 0.8D0+0.3D0*(i-1)    ! n=13 for Xi
!    pt = 0.4D0+0.1D0*(i-1) ! n=41 for phi
!    pt= 0.9D0+0.1D0*(i-1)  !n=36 for Omega
...
!	gamai = 0.07d0
!   For Pb+Pb, 2.76 TeV JPG47, 055102(2020)
   gamai = 1.4d0/(1.0d0+q**2/49.0d0) ! consider the gammai depends on q
!   For Pb+Pb, 5.02 TeV
!	gamai = 2.25d0/(1.0d0+q**2/49.0d0)
	endif
